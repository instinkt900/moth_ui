...
SDLUIRenderer::SDLUIRenderer(SDL_Renderer& renderer)
    : m_renderer(renderer) {
    m_drawColor.push({ 1.0f, 1.0f, 1.0f, 1.0f });
    m_blendMode.push(moth_ui::BlendMode::Replace);
}

void SDLUIRenderer::PushBlendMode(moth_ui::BlendMode mode) {
    m_blendMode.push(mode);
}

void SDLUIRenderer::PopBlendMode() {
    if (m_blendMode.size() > 1) {
        m_blendMode.pop();
    }
}

void SDLUIRenderer::PushColor(moth_ui::Color const& color) {
    auto const modColor = m_drawColor.top() * color;
    m_drawColor.push(modColor);
}

void SDLUIRenderer::PopColor() {
...
void SDLUIRenderer::PushClip(moth_ui::IntRect const& rect) {
    if (m_clip.empty()) {
        m_clip.push(rect);
    } else {
...
void SDLUIRenderer::RenderRect(moth_ui::IntRect const& rect) {
...
void SDLUIRenderer::RenderFilledRect(moth_ui::IntRect const& rect) {
    auto const sdlRect{ ToSDL(rect) };
    ColorComponents components{ m_drawColor.top() };
    SDL_SetRenderDrawBlendMode(&m_renderer, ToSDL(m_blendMode.top()));
    SDL_SetRenderDrawColor(&m_renderer, components.r, components.g, components.b, components.a);
...
void SDLUIRenderer::RenderImage(moth_ui::IImage& image, moth_ui::IntRect const& sourceRect, moth_ui::IntRect const& destRect, moth_ui::ImageScaleType scaleType, float scale) {
    auto const& internalImage = static_cast<SDLImage&>(image);
...
void SDLUIRenderer::RenderText(std::string const& text, moth_ui::IFont& font, moth_ui::TextHorizAlignment horizontalAlignment, moth_ui::TextVertAlignment verticalAlignment, moth_ui::IntRect const& destRect) {
    auto const fcFont = static_cast<SDLFont&>(font).GetFontObj();
