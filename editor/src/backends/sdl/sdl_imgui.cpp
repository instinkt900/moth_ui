...
namespace imgui_ext {
	void Image(std::shared_ptr<moth_ui::IImage> image, int width, int height){
		...
		ImGui::Image(imageTexture->GetImpl(), ImVec2(static_cast<float>(width), static_cast<float>(height)));
	}
    ...
    void Inspect(char const* name, moth_ui::IImage const* image) {
        ...
        ImGui::Image(sdlTexture->GetImpl(), ImVec2(scaledDimensions.x, scaledDimensions.y), ImVec2(uv0.x, uv0.y), ImVec2(uv1.x, uv1.y));
    }
}
