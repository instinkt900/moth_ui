...
SDLFontFactory::SDLFontFactory(SDL_Renderer& renderer)
    : m_renderer(renderer) {
...
std::unique_ptr<moth_ui::IFont> SDLFontFactory::GetDefaultFont(int size) {
    return GetFont(m_fontPaths.begin()->first.c_str(), size);
}

std::vector<std::string> SDLFontFactory::GetFontNameList() {
...
std::unique_ptr<moth_ui::IFont> SDLFontFactory::GetFont(char const* name, int size) {
    ...
    return std::make_unique<SDLFont>(CreateCachedFontRef(&m_renderer, it->second.string().c_str(), size, defaultColor, TTF_STYLE_NORMAL));
}
